...
decode_file(File) ->
  ...
  case Res of
    {ok, Bin} ->
      {ok, Version,Tempo,Rest} = parse_header(Bin),
...
render_tracks([{TrackN, Instrument, Measure}|Rest],Padding, Acc) ->
  render_tracks(Rest,Padding,
    ...
    [TrackN,Instrument,lists:duplicate(Padding-length(Instrument)-length(integer_to_list(TrackN)),$ ),render_quarter(Measure)])|Acc]).
